...
const formData = reactive({
  ...
  friend_apply_auto_accept_on: 0,
  privilege_only_create_invite_group_on: 0
});
//  初始化数据
const initData = () => {
  getAppconfigGet().then((res: any) => {
    ...
    formData.friend_apply_auto_accept_on = Number(res.friend_apply_auto_accept_on ?? 0);
    formData.privilege_only_create_invite_group_on = Number(res.privilege_only_create_invite_group_on ?? 0);
  });
};
